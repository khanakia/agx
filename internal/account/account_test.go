...
func TestDiscover_Empty(t *testing.T) {
	...
	if err != nil || len(got) != 0 {
		t.Errorf("Discover(empty) = %v, %v", got, err)
	}
}

func TestIsDefaultDir(t *testing.T) {
	t.Parallel()
	if !IsDefaultDir("/h/.claude/", "/h") || IsDefaultDir("/h/.claude-work", "/h") {
		t.Error("IsDefaultDir wrong")
	}
}
...
func TestLoadProfile(t *testing.T) {
	...
	if p := LoadProfile(def, home, true); p.Email != "me@example.com" || p.Organization != "Mine" {
		t.Errorf("default profile = %+v", p)
	}
	if p := LoadProfile(work, home, false); p.Email != "me@work.example" {
		t.Errorf("work profile = %+v", p)
	}
	// A non-default dir must not borrow ~/.claude.json's identity.
	if p := LoadProfile(filepath.Join(home, ".claude-empty"), home, false); p != (Profile{}) {
		t.Errorf("missing profile = %+v, want empty", p)
	}
}
