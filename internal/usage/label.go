...
// GroupHeading is the section title for a group, or "" for the session group,
// which renders without a heading.
func GroupHeading(g Group) string {
	if g == GroupSession {
		return ""
	}
	if h, ok := groupHeadings[g]; ok {
		return h
	}
	return string(g)
}
...
// beside the OAuth token. Either may be empty; an empty subscription yields ""
// so the caller can omit the plan rather than print a blank one.
